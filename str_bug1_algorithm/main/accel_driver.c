...
static esp_err_t accel_driver_write_byte(uint8_t reg, uint8_t data)
{
    ...
    return i2c_master_write_to_device(ACCEL_DRIVER_I2C_MASTER_PORT, ACCEL_DRIVER_MPU6050_I2C_ADDR, buf, 2,
                                      1000 / portTICK_PERIOD_MS);
}

static esp_err_t accel_driver_read_bytes(uint8_t reg, uint8_t* data, size_t len)
{
    return i2c_master_write_read_device(ACCEL_DRIVER_I2C_MASTER_PORT, ACCEL_DRIVER_MPU6050_I2C_ADDR, &reg, 1, data, len,
                                        1000 / portTICK_PERIOD_MS);
}
...
esp_err_t accel_driver_get_values(struct accel_driver_values_ms* values)
{
    ...
    err = accel_driver_read_bytes(0x3B, data, 6);
